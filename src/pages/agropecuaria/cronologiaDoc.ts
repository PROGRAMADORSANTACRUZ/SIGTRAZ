...
const ESTILOS =
  ...
  `body{font-family:'Segoe UI',Arial,sans-serif;color:#1f2937;background:#e2e8f0;}` +
  `.barra{position:fixed;top:0;left:0;right:0;background:#1e293b;padding:8px 16px;display:flex;gap:8px;justify-content:flex-end;z-index:20;}` +
  ...
  `.hoja{position:relative;width:816px;min-height:1056px;margin:70px auto;background:#fff;box-shadow:0 2px 12px rgba(0,0,0,.2);}` +
  `.fondo{position:absolute;inset:0;width:100%;height:100%;object-fit:fill;z-index:0;pointer-events:none;}` +
  `.contenido{position:relative;z-index:1;padding:1.95in 0.7in 1.4in;}` +
  ...
  `table.datos tbody tr{page-break-inside:avoid;}` +
  `table.datos tbody tr:nth-child(even){background:#f1f8e9;}` +
  `.firma{margin-top:90px;text-align:center;page-break-inside:avoid;}` +
  ...
  `.barra{display:none;}` +
  `body{background:#fff;}` +
  `.hoja{position:static;width:auto;min-height:0;margin:0;box-shadow:none;}` +
  `.fondo{position:fixed;top:0;left:0;width:100%;height:100vh;object-fit:fill;}` +
  `}`
