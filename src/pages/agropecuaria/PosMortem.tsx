...
export function PosMortem() {
  ...
  async function exportarExcel() {
    ...
    const ws = wb.addWorksheet('Pos Mortem')
    const borde = {
    ...
    const filaTitulo = ws.addRow(['INSPECCION POST MORTEM BOVINOS'])
    ...
    enlace.download = 'pos-mortem.xlsx'
    enlace.click()
  ...
  function exportarPDF() {
    ...
    if (!win) return
    win.document.write(
      `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Pos Mortem</title><style>body{font-family:Arial,sans-serif;padding:16px;}h1{font-size:16px;}table{border-collapse:collapse;width:100%;font-size:9px;}th,td{border:1px solid #94a3b8;padding:4px;text-align:left;}th{background:#e2e8f0;}</style></head><body><h1>INSPECCION POST MORTEM BOVINOS</h1><table><thead><tr>${encabezado}</tr></thead><tbody>${cuerpo}</tbody></table><script>window.onload=function(){window.print();}</script></body></html>`,
    )
    win.document.close()
  }
  ...
  function generarCertificado(items: RegistroPosMortem[]) {
    ...
    const total = ante
      ? num(ante.novillo) + num(ante.vaca) + num(ante.toro) + num(ante.bufalo)
      : 0
    const soloBufalo =
      !!ante &&
      num(ante.bufalo) > 0 &&
      num(ante.novillo) + num(ante.vaca) + num(ante.toro) === 0
    const tipo = soloBufalo ? 'BUFALOS' : 'BOVINOS'
    ...
    // el mismo consecutivo (CDB-N) en lugar de crear otro.
