...
import { api, type NuevoResiduoReciclable } from '../services/api'
import { ModalEliminar } from '../components/ModalEliminar'
import { FirmaCanvas } from '../components/FirmaCanvas'
import type { ResiduoReciclable } from '../types/trazabilidad'

interface FormReciclable {
  fecha: string
  material: string
  cantidad: string
...
const formVacio = (): FormReciclable => ({
...
export function ResiduosReciclables() {
  const [registros, setRegistros] = useState<ResiduoReciclable[]>([])
  const [cargando, setCargando] = useState(true)
  const [error, setError] = useState<string | null>(null)

  const [mostrarForm, setMostrarForm] = useState(false)
  const [editandoId, setEditandoId] = useState<string | null>(null)
  const [form, setForm] = useState<FormReciclable>(formVacio)
  const [guardando, setGuardando] = useState(false)
  const [errorForm, setErrorForm] = useState<string | null>(null)
  const [busqueda, setBusqueda] = useState('')

  const [aEliminar, setAEliminar] = useState<ResiduoReciclable | null>(null)
  ...
  async function cargar() {
    ...
    setError(null)
    try {
      const datos = await api.getResiduosReciclables()
      setRegistros(datos)
    } catch (err) {
  ...
  const formValido = useMemo(
    () =>
      form.fecha.trim() !== '' &&
      form.firmaRecibe.trim() !== '' &&
      form.firmaRecibeImagen.trim() !== '',
    [form],
  )
  ...
  const registrosFiltrados = useMemo(() => {
    ...
    return registros.filter(
      (r) =>
        ...
        (r.entidadRecolectora ?? '').toLowerCase().includes(t) ||
        (r.observaciones ?? '').toLowerCase().includes(t),
    )
  }, [registros, busqueda])

  function actualizar<K extends keyof FormReciclable>(
    campo: K,
    valor: FormReciclable[K],
  ) {
  ...
  function abrirEdicion(r: ResiduoReciclable) {
    setEditandoId(r.id)
    setForm({
      fecha: r.fecha ?? '',
      material: r.material ?? '',
      cantidad: r.cantidad ?? '',
      entidadRecolectora: r.entidadRecolectora ?? '',
      firmaEntrega: r.firmaEntrega ?? '',
      firmaRecibe: r.firmaRecibe ?? '',
      firmaRecibeImagen: r.firmaRecibeImagen ?? '',
  ...
  async function guardar(e: React.FormEvent) {
    ...
    setErrorForm(null)
    try {
      const datos: NuevoResiduoReciclable = {
        fecha: form.fecha || undefined,
        material: form.material.trim() || undefined,
        cantidad: form.cantidad.trim() || undefined,
        entidadRecolectora: form.entidadRecolectora.trim() || undefined,
        firmaEntrega: form.firmaEntrega.trim() || undefined,
        firmaRecibe: form.firmaRecibe.trim() || undefined,
        firmaRecibeImagen: form.firmaRecibeImagen.trim() || undefined,
        observaciones: form.observaciones.trim() || undefined,
      }
      if (editandoId) {
        const actualizado = await api.actualizarResiduoReciclable(
          editandoId,
          datos,
        )
        setRegistros((prev) =>
        ...
        const creado = await api.crearResiduoReciclable(datos)
  ...
  async function eliminar(password: string) {
    ...
    try {
      await api.eliminarResiduoReciclable(aEliminar.id, password)
  ...
  function imprimir(filas: ResiduoReciclable[]) {
      ...
      .map(
        (r) => `
          <tr>
            ...
            <td>${cel(r.material)}</td>
            <td>${cel(r.cantidad)}</td>
            <td>${cel(r.entidadRecolectora)}</td>
            <td>${cel(r.firmaEntrega)}</td>
            <td>${cel(r.firmaRecibe)}</td>
            <td class="firma">${
              r.firmaRecibeImagen
                ? `<img src="${r.firmaRecibeImagen}" alt="firma">`
                : '&nbsp;'
            }</td>
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8">
      <title>Evacuacion de Residuos Reciclables</title>
      <style>
      ...
      <table class="cab">
        <tr>
          ...
          <td class="tit">FORMATO DE EVACUACION DE RESIDUOS RECICLABLES</td>
          <td class="cod">CODIGO: FOR-CIA-019</td>
        </tr>
        <tr>
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>ENTIDAD RECOLECTORA</th>
            <th>FIRMA QUIEN ENTREGA</th>
            <th>NOMBRE QUIEN RECIBE</th>
            <th>FIRMA QUIEN RECIBE</th>
            <th>OBSERVACIONES</th>
          </tr>
        </thead>
  ...
  async function exportarExcel(filas: ResiduoReciclable[]) {
    if (filas.length === 0) return
    const wb = new ExcelJS.Workbook()
    const ws = wb.addWorksheet('Residuos Reciclables', {
      views: [{ showGridLines: false }],
    })

    const anchos = [14, 22, 16, 24, 24, 24, 30]
    anchos.forEach((w, i) => {
    ...
    ws.mergeCells('C1:E3')
    ws.getCell('C1').value =
      'FORMATO DE EVACUACION DE RESIDUOS RECICLABLES\nPROGRAMA DE TRATAMIENTO DE RESIDUOS SOLIDOS Y LIQUIDOS'
    ws.getCell('F1').value = 'CODIGO: FOR-CIA-019'
    ws.getCell('F2').value = 'VERSION: 0'
    ws.getCell('F3').value = 'FECHA: 08/11/2021'
    ws.mergeCells('F1:G1')
    ws.mergeCells('F2:G2')
    ws.mergeCells('F3:G3')
    ...
    ws.getCell('B4').value = 'MATERIAL'
    ws.getCell('C4').value = 'CANTIDAD kg / Und'
    ws.getCell('D4').value = 'ENTIDAD RECOLECTORA'
    ws.getCell('E4').value = 'FIRMA QUIEN ENTREGA'
    ws.getCell('F4').value = 'FIRMA QUIEN RECIBE'
    ...
    enlace.download =
      filas.length === 1
        ? `residuos-reciclables-${filas[0].material || filas[0].id}.xlsx`
        : `residuos-reciclables-${filas.length}.xlsx`
    enlace.href = url
    enlace.click()
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-end justify-between">
        <div>
          ...
          <p className="text-slate-500">
            Evacuacion de residuos reciclables - Programa de tratamiento de
            residuos (FOR-CIA-019)
          </p>
        </div>
        <button
          onClick={abrirNuevo}
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-3">
        ...
        <Kpi
          label="Materiales"
          value={new Set(registros.map((r) => r.material).filter(Boolean)).size}
        />
      ...
      {mostrarForm && (
        ...
        >
          ...
          <div className="md:col-span-3 grid grid-cols-1 gap-4 md:grid-cols-5">
            <Campo label="Fecha *">
              <input
                type="date"
                value={form.fecha}
            ...
            <Campo label="Material">
              <input
                ...
                placeholder="Ej. Carton, plastico..."
                className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500"
              />
            </Campo>

            <Campo label="Cantidad kg / Und">
              <input
                value={form.cantidad}
            ...
            <Campo label="Entidad recolectora">
              <input
                value={form.entidadRecolectora}
                onChange={(e) =>
                  actualizar('entidadRecolectora', e.target.value)
                }
            ...
            <Campo label="Firma quien entrega">
              <input
                value={form.firmaEntrega}
                onChange={(e) => actualizar('firmaEntrega', e.target.value)}
          ...
          <Campo label="Firma quien recibe *">
            <input
              value={form.firmaRecibe}
              onChange={(e) => actualizar('firmaRecibe', e.target.value)}
          ...
          <div className="block text-sm">
            <span className="mb-1 block font-medium text-slate-700">
              Firma de quien recibe (dibuja con el mouse) *
            </span>
            <FirmaCanvas
              value={form.firmaRecibeImagen}
              onChange={(dataUrl) => actualizar('firmaRecibeImagen', dataUrl)}
            />
          </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <input
          ...
          placeholder="Buscar por material, entidad u observaciones..."
        ...
        <table className="w-full text-sm">
          <thead className="sticky top-0 z-10 bg-slate-50 text-left text-slate-500 shadow-sm">
            <tr>
              ...
              <th className="px-4 py-3 font-medium">Cantidad kg / Und</th>
              <th className="px-4 py-3 font-medium">Entidad recolectora</th>
              <th className="px-4 py-3 font-medium">Firma entrega</th>
              <th className="px-4 py-3 font-medium">Firma recibe</th>
                  ...
                  {cargando
                    ...
                    : error
                      ...
                      : busqueda
                        ...
                        : 'Sin registros de residuos reciclables.'}
                </td>
              </tr>
            )}
          </tbody>
