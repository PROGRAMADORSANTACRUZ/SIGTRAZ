...
export function TrazabilidadAcondicionamiento() {
  ...
  useEffect(() => {
    let activo = true
    setEstado('cargando')
    fetch(`${API_URL}/trazabilidad/acond/${id}`)
      .then((r) => {
        ...
        {estado === 'ok' && traza && (
          <div className="space-y-5 px-6 py-6">
            ...
            <Seccion titulo="Proceso">
              {traza.lote && <Dato etiqueta="Lote origen" valor={traza.lote} />}
              {traza.proceso && (
                <Dato etiqueta="Proceso" valor={traza.proceso} />
              )}
              {traza.responsable && (
                <Dato etiqueta="Responsable" valor={traza.responsable} />
              )}
              {traza.empresa && (
                <Dato etiqueta="Empacado por" valor={traza.empresa} />
              )}
            </Seccion>

            {/* Fechas */}
            <Seccion titulo="Fechas">
              <Dato etiqueta="Fecha de empaque" valor={fmtFecha(traza.fechaEmpaque)} />
              <Dato
              ...
              />
              {traza.vidaUtilDias != null && (
                <Dato etiqueta="Vida útil" valor={`${traza.vidaUtilDias} días`} />
              )}
              <Dato
            ...
            {/* Conservación */}
            {(traza.conservacion || traza.instrucciones) && (
              <Seccion titulo="Conservación y uso">
                {traza.conservacion && (
                  <Dato etiqueta="Conservación" valor={traza.conservacion} />
                )}
                {traza.instrucciones && (
                  <Dato etiqueta="Instrucciones" valor={traza.instrucciones} />
                )}
              </Seccion>
            )}
